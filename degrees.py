...
def shortest_path(source, target):
    ...
    # Create a frontier using a QueueFrontier
    ...
    while not frontier.empty():
        ...
        # Check if the node's state is the target
        if node.state == target:
            # Create a list to store the path from source to target
            path = []
            while node.parent is not None:
                # Add the (movie_id, person_id) pair to the path
